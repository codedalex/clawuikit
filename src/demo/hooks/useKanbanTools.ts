...
export function useKanbanTools(actions: KanbanActions, state: KanbanState) {
  ...
  const columnSummary = () =>
    state.columns.map((c) => `${c.title} (id: ${c.id})`).join(", ");
  ...
  useCopilotAction({
    ...
    handler: async ({ columnId, title, description }) => {
      ...
      const id = actions.addCard(columnId, title, description ?? "");
  ...
  useCopilotAction({
    ...
    handler: async ({ cardId, toColumnId }) => {
      ...
      const col = state.columns.find((c) => c.id === toColumnId);
      if (!card)
  ...
  useCopilotAction({
    ...
    handler: async ({ cardId, title, description }) => {
      ...
      actions.updateCard(cardId, patch);
  ...
  useCopilotAction({
    ...
    handler: async ({ title }) => {
      const id = actions.addColumn(title);
  ...
  useCopilotAction({
    ...
    handler: async ({ columnId, title }) => {
      ...
      if (!col)
        return `Error: column "${columnId}" not found. Available: ${columnSummary()}`;
      actions.renameColumn(columnId, title);
